...
<script>
import { listUserPointHistory } from '@/api/user'
  ...
  methods: {
    getList() {
      if (this.listQuery.userId || (this.listQuery.start && this.listQuery.end)) {
        ...
        listUserPointHistory(this.listQuery)
          .then(response => {
    ...
    handleDownload() {
      ...
      import('@/vendor/Export2Excel').then(excel => {
        const tHeader = [ 'ID', '用户ID', '积分', '类型', '添加时间']
        const filterVal = ['id', 'userId', 'points', 'reason', 'addTime']
